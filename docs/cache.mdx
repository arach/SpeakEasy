---
title: Cache System
description: Intelligent caching for faster TTS playback
---
...
- **Uses built-in SQLite** (`node:sqlite`) — no extra dependencies
...
```
/tmp/speakeasy-cache/
├── cache.sqlite               # SQLite metadata database (node:sqlite or bun:sqlite)
...
Legacy `metadata.sqlite` and `tts-cache.sqlite` files from the old `better-sqlite3` cache are automatically migrated on first run.
...
1. **Sorts by creation time** (oldest first)
2. **Deletes oldest entries** until under the limit
...
```typescript
// Good: Good - same provider/voice/rate hits cache
...
// Bad: Bad - different rate misses cache
await speaker.speak('Hello', { provider: 'openai', voice: 'nova', rate: 180 });
