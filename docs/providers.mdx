---
title: Providers
description: TTS provider setup and configuration
---
...
To add your own TTS backend (local service, private API, experimental model), see the [Custom Providers Guide](custom-providers.md). It covers the `TTSAdapter` contract, standalone usage without forking, and the full wiring checklist for first-class CLI/SDK integration.