...
description: Permanent macOS player, HUD, and playback IPC
---
...
**Release:** included in GitHub **SpeakEasy 0.2.17** (signed DMG). Install with
`speakeasy --app` or download from
[GitHub Releases](https://github.com/arach/SpeakEasy/releases/latest).

Listening mode (thread-locked Codex conversations and the dedicated microphone
picker) is documented separately in [Listening mode](./listening-mode) and ships
in app sources merged after the 0.2.17 cut.
...
```bash
speakeasy --app          # install from GitHub Releases if needed, then open
speakeasy --update-app   # pull the latest signed DMG
```

Requirements: macOS 14 or newer; Developer ID signed and notarized release
builds are published as `SpeakEasy.dmg` / `SpeakEasy-<version>.dmg`.
...
- [Listening mode](./listening-mode) — thread-locked Codex voice turns and mic picker
- [CLI](./cli) — synthesis and `--app` helpers
- [SDK](./sdk) — programmatic speech
- [Troubleshooting](./troubleshooting) — player and permission issues
