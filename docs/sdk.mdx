...
const all = await speaker.getCacheMetadata();
const matches = await speaker.findByText('build completed');
const openai = await speaker.findByProvider('openai');
...
The SDK exports the adapter primitives for bespoke TTS backends:
...
import {
  ...
  TTSCache,
} from '@arach/speakeasy';
```

Use adapters standalone (synthesize + play) or wire them into SpeakEasy's registry for first-class CLI support. See [Custom Providers](custom-providers.md).
