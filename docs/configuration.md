...
### Deck Bridge Settings

Optional defaults for `speakeasy deck` (CLI flags always win):

| Key | Type | Effect |
|---|---|---|
| `deck.port` | number | Preferred deck port. Falls back to auto (80 when free, else 43211+) when busy — unlike `--port`, which must be bindable. |
...
deck status and can start/stop/restart the bridge.
